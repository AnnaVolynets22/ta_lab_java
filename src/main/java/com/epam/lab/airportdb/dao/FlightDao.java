...
public class FlightDao implements Dao<Flight> {
    ...
    public Optional<Flight> get(String id) throws SQLException {
        Flight flight = null;
        try (PreparedStatement ps = ConnectionHandler.getConnection().prepareStatement(FIND_FRIGHT_BY_ID)) {
            ps.setString(1, id);
    ...
    public List<Flight> getAll() throws SQLException {
        ...
        try (PreparedStatement ps = ConnectionHandler.getConnection().prepareStatement(FIND_ALL_FLIGHTS)) {
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    City departureCity = cityDao.get(rs.getString("departureCity")).get();
                    City destinationCity = cityDao.get(rs.getString("destinationCity")).get();
